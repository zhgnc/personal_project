...
| quat()              | std::initializer_list<type> initial_quat   | quat         | Construct quaternion with initializer list                         | quat<double> q = {0, 0, 0, 1};          |
| quat()              | const quat<type>& another_quaternion       | quat         | Construct quaternion and set it equal to another quaternion        | quat<double> q2 = q1;                   |
| operator()          | std::size_t element_to_return           | type&           | Access element by index for modification                           | q(0) = 0.5;                             |
| operator() const    | std::size_t element_to_return           | const type&     | Access element by index for reading                                | double scalar = q(3);                   |
| operator=           | const quat<type>& another_quaternion       | quat&        | Assignment operator                                                | q1 = q2;                                |
...
| scalar()            | None                                    | type            | Returns the scalar (real) part                                     | double scalar = q.scalar();             |
| vec()               | None                                    | vector<type,3>  | Returns the vector (imaginary) part                                | vector<double,3> v = q.vec();           |
| setVector()         | vector<type,3> vector                      | void         | Sets the vector (imaginary) part                                   | q.setVector(v);                         |
| setScalar()         | type scalar                                | void         | Sets the scalar (real) part                                        | q.setScalar(1.0);                       |
...
| operator*           | const quat<type>& q_12                     | quat         | Quaternion multiplication (active rotation composition)            | q_13 = q_23 * q_12;                     |
| operator*           | const vector<type,3>& vector               | vector<type,3>  | Rotates a 3D vector using the quaternion                           | vector<double,3> v2 = q * v1;           |
