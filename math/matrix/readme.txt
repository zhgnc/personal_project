...
| diag_matrix()           | const matrix<T, rows, 1>& matrix or const vector<T, rows>& vec   | matrix        | Creates a diagonal matrix from the input data   | matrix<int,3,3> m = diag_matrix({1,2,3});     |
| skew_matrix()           | const matrix<T, rows, 1>& matrix or const vector<T, rows>& vec   | matrix        | Creates a skew symetric matrix A so that A^T=-A | matrix<int,3,3> m = skew_matrix<int,3>(vec);  |
...
| set_skew()              | const matrix<T, rows, 1>& matrix or const vector<T, rows>& vec   | void          | Sets matrix to row major skew symetric matrix   | m.set_skew({1,2,3,4});                        |
...
| element_wise_multiply() | const matrix<T, rows, columns>& matrix                           | matrix        | Element-wise multiplication                     | C = A.element_wise_multiply(B);               |
| element_wise_division() | const matrix<T, rows, columns>& matrix                           | matrix        | Element-wise division                           | C = A.element_wise_division(B);               |
