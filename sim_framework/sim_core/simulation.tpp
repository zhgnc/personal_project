...
Simulation<DataBusType>::Simulation(const std::string &path_to_sim_config, DataBusType& bus) : data_bus(bus) {
  ...
  start_time_sec        = get_yaml_value<double>(config_data, "sim_start_time_sec");
  config_stop_time_sec  = get_yaml_value<double>(config_data, "sim_stop_time_sec");
  sim_rate_hz           = get_yaml_value<double>(config_data, "simulation_rate_hz");
  num_mc_runs           = get_yaml_value<std::size_t>(config_data, "number_of_monte_carlo_runs");
  init_seed             = get_yaml_value<uint64_t> (config_data, "initial_random_seed");
  ...
  data_output_directory   = get_yaml_value<std::string>(config_data, "logging_file_save_directory");
  base_file_name          = get_yaml_value<std::string>(config_data, "logging_filename_prefix");
