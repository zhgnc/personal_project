...
template<typename T>
long long det3x3_exact(const Matrix2D<T,3,3>& A) {
...
template<typename T>
long double det4x4_exact(const Matrix2D<T,4,4>& A) {
    ...
    auto det3 = [](long double b00,long double b01,long double b02,
                   ...
                   long double b20,long double b21,long double b22) -> long double {
    ...
    long double M11 = det3(
        ld(A(1,1)), ld(A(1,2)), ld(A(1,3)),
        ld(A(2,1)), ld(A(2,2)), ld(A(2,3)),
        ld(A(3,1)), ld(A(3,2)), ld(A(3,3))
    );

    long double M12 = det3(
        ld(A(1,0)), ld(A(1,2)), ld(A(1,3)),
        ld(A(2,0)), ld(A(2,2)), ld(A(2,3)),
        ld(A(3,0)), ld(A(3,2)), ld(A(3,3))
    );

    long double M13 = det3(
        ld(A(1,0)), ld(A(1,1)), ld(A(1,3)),
        ld(A(2,0)), ld(A(2,1)), ld(A(2,3)),
        ld(A(3,0)), ld(A(3,1)), ld(A(3,3))
    );

    long double M14 = det3(
        ld(A(1,0)), ld(A(1,1)), ld(A(1,2)),
        ld(A(2,0)), ld(A(2,1)), ld(A(2,2)),
        ld(A(3,0)), ld(A(3,1)), ld(A(3,2))
    );
...
int main() {
  ...
  Matrix2D<double, 3, 3> I = {1, 0, 0,
                            0, 1, 0,
                            0, 0, 1};
  std::cout << "det(I) = " << I.det() << " (expected 1)\n";

  Matrix2D<double, 3, 3> D = {2, 0, 0,
                            0, 3, 0,
                            0, 0, 4};
  std::cout << "det(D) = " << D.det() << " (expected 24)\n";

  Matrix2D<double, 3, 3> U = {1, 2, 3,
                            0, 4, 5,
                            0, 0, 6};
  std::cout << "det(U) = " << U.det() << " (expected 24)\n";

  Matrix2D<double, 2, 2> S = {2, 4,
                            1, 2};
  std::cout << "det(S) = " << S.det() << " (expected 0)\n";

  Matrix2D<double, 2, 2> N = {0, 1,
                            1, 0};
