...
public:
    ...
    void configure_model(const std::string& path_to_config, SimControl& sim_ctrl) override {
        ...
        sim_ctrl.write_attribute<double>("test_group",   "init_sim_dt_sec",           sim_ctrl.public_sim_data().sim_dt_sec);
        sim_ctrl.write_attribute<double>("test_group",   "init_sim_rate_hz",          sim_ctrl.public_sim_data().sim_rate_hz);        
        sim_ctrl.write_attribute<uint64_t>("test_group", "init_sim_step_count",       sim_ctrl.public_sim_data().sim_step_count);
    };
